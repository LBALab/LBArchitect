...
//Taken from https://stackoverflow.com/questions/9466547/how-to-make-a-combo-box-with-fulltext-search-autocomplete-support
//and some bugs fixed
...
begin
  ...
  if dofilter and not (ord(key) in [13,27]) then begin
    {if droppeddown then begin
      FilterItems;
      if (items.Count > previtems) and (previtems < DropDownCount) then
        SendMessage(Handle, CB_SHOWDROPDOWN, 0, 0); //close up combo to recalculate the visible items count
      previtems:= items.Count;
    end;} //-- tried to fix low height of the box on backspace press after re-drop, but Text is still not updated here,
    //         so probably not possible in Delphi
...
begin
  ...
  dofilter:= true;
  storeditemindex:= -1;
  //SendMessage(Handle, CB_SETMINVISIBLE, 20, 0); -- supported on WinVista+
end;

procedure TSmartComboBox.ClearSmartCombo;
begin
  Clear();
  FStoredItems.OnChange:= nil;
  StoredItems.Clear();
