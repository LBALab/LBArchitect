...
type
  TSynTrackScript1HL = class(TSynCustomHighlighter)
  private
    fLineRef: string;
    fLine: PChar;
    fLineNumber: Integer;
    ...
    {$I scTrackScript1IdentHeads.inc}
    procedure IdentProc;
...
procedure TSynTrackScript1HL.InitIdent;
var
  I: Integer;
...
begin
  ...
  {$I scTrackScript1IdentTable.inc}
end;

function TSynTrackScript1HL.KeyHash(ToHash: PChar): Integer;
begin
  Result := 0;
...
function TSynTrackScript1HL.KeyComp(const aKey: String): Boolean;
var
  I: Integer;
  Temp: PChar;
begin
...
{$I scTrackScript1IdentFuncs.inc}

function TSynTrackScript1HL.AltFunc: TtkTokenKind;
begin
  Result := tkIdentifier;
end;

function TSynTrackScript1HL.IdentKind(MayBe: PChar): TtkTokenKind;
var
  HashKey: Integer;
begin
  fToIdent := MayBe;
...
procedure TSynTrackScript1HL.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
procedure TSynTrackScript1HL.SpaceProc;
begin
...
procedure TSynTrackScript1HL.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TSynTrackScript1HL.IntegerProc;
begin
 Inc(Run);
 fTokenID := tkNumber;
...
procedure TSynTrackScript1HL.CRProc;
begin
  fTokenID := tkSpace;
  inc(Run);
...
procedure TSynTrackScript1HL.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynTrackScript1HL.BraceCommentOpenProc;
begin
  Inc(Run);
...
procedure TSynTrackScript1HL.BraceCommentProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynTrackScript1HL.CStyleCommentOpenProc;
begin
  Inc(Run);
  {if (fLine[Run] = '*') then
  begin
...
procedure TSynTrackScript1HL.CStyleSingleLineProc;
begin
  fTokenID := tkComment;
  repeat
    Inc(Run);
...
procedure TSynTrackScript1HL.REMCommentOpenProc;
begin
...
constructor TSynTrackScript1HL.Create(AOwner: TComponent);
begin
...
procedure TSynTrackScript1HL.SetLine(NewValue: String; LineNumber: Integer);
begin
  fLineRef := NewValue;
...
procedure TSynTrackScript1HL.IdentProc;
begin
...
procedure TSynTrackScript1HL.UnknownProc;
begin
...
procedure TSynTrackScript1HL.Next;
begin
  fTokenPos := Run;
  case fRange of
...
function TSynTrackScript1HL.GetDefaultAttribute(Index: integer): TSynHighLighterAttributes;
begin
  case Index of
...
function TSynTrackScript1HL.GetEol: Boolean;
begin
  Result := fTokenID = tkNull;
end;

function TSynTrackScript1HL.GetKeyWords: string;
begin
  Result := 'LABEL,GOTO,END,STOP';
end;

function TSynTrackScript1HL.GetToken: String;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
function TSynTrackScript1HL.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TSynTrackScript1HL.GetTokenAttribute: TSynHighLighterAttributes;
begin
  case GetTokenID of
...
function TSynTrackScript1HL.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynTrackScript1HL.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

function TSynTrackScript1HL.GetIdentChars: TSynIdentChars;
begin
  Result := ['_', 'a'..'z', 'A'..'Z'];
end;

function TSynTrackScript1HL.GetSampleSource: string;
begin
  Result := '';
end;

function TSynTrackScript1HL.IsFilterStored: Boolean;
begin
  Result := fDefaultFilter <> SYNS_FilterTrackScript;
end;

{$IFNDEF SYN_CPPB_1} class {$ENDIF}
function TSynTrackScript1HL.GetLanguageName: string;
begin
  Result := SYNS_LangTrackScript;
end;

procedure TSynTrackScript1HL.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynTrackScript1HL.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

function TSynTrackScript1HL.GetRange: Pointer;
begin
...
initialization
  MakeIdentTable;
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynTrackScript1HL);
{$ENDIF}
end.
