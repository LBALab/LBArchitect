...
type
  TSynLifeScript2HL = class(TSynCustomHighlighter)
  private
    fLineRef: string;
    fLine: PChar;
    fLineNumber: Integer;
...
procedure TSynLifeScript2HL.InitIdent;
var
  I: Integer;
...
begin
  ...
  {$I scLifeScript2IdentTable.inc}
end;

function TSynLifeScript2HL.KeyHash(ToHash: PChar): Integer;
begin
  Result := 0;
...
function TSynLifeScript2HL.KeyComp(const aKey: String): Boolean;
var
  I: Integer;
  Temp: PChar;
begin
...
{$I scLifeScript2IdentFuncs.inc}

function TSynLifeScript2HL.AltFunc: TtkTokenKind;
begin
  Result := tkIdentifier;
end;

function TSynLifeScript2HL.IdentKind(MayBe: PChar): TtkTokenKind;
var
  HashKey: Integer;
begin
  fToIdent := MayBe;
...
procedure TSynLifeScript2HL.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
procedure TSynLifeScript2HL.SpaceProc;
begin
...
procedure TSynLifeScript2HL.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TSynLifeScript2HL.OperatorProc;
begin
 fTokenID:= tkOperator;
 case fLine[Run] of
...
procedure TSynLifeScript2HL.IntegerProc;
begin
 Inc(Run);
 fTokenID := tkNumber;
...
procedure TSynLifeScript2HL.CRProc;
begin
  fTokenID := tkSpace;
  inc(Run);
...
procedure TSynLifeScript2HL.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynLifeScript2HL.BraceCommentOpenProc;
begin
  Inc(Run);
...
procedure TSynLifeScript2HL.BraceCommentProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynLifeScript2HL.CStyleCommentOpenProc;
begin
  Inc(Run);
  {if (fLine[Run] = '*') then
  begin
...
{procedure TSynLifeScript2HL.CStyleCommentProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
{procedure TSynLifeScript2HL.DelphiCommentOpenProc;
begin
  Inc(Run);
  if (fLine[Run] = '*') then
  begin
...
{procedure TSynLifeScript2HL.DelphiCommentProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynLifeScript2HL.CStyleSingleLineProc;
begin
  fTokenID := tkComment;
  repeat
    Inc(Run);
...
procedure TSynLifeScript2HL.REMCommentOpenProc;
begin
...
constructor TSynLifeScript2HL.Create(AOwner: TComponent);
begin
...
procedure TSynLifeScript2HL.SetLine(NewValue: String; LineNumber: Integer);
begin
  fLineRef := NewValue;
...
procedure TSynLifeScript2HL.IdentProc;
begin
...
procedure TSynLifeScript2HL.UnknownProc;
begin
...
procedure TSynLifeScript2HL.Next;
begin
  fTokenPos := Run;
  case fRange of
...
function TSynLifeScript2HL.GetDefaultAttribute(Index: integer): TSynHighLighterAttributes;
begin
  case Index of
...
function TSynLifeScript2HL.GetEol: Boolean;
begin
  Result := fTokenID = tkNull;
end;

function TSynLifeScript2HL.GetKeyWords: string;
begin
  Result :=
    'COMPORTMENT,ELSE,END,END_COMPORTMENT,ENDIF,IF,ONEIF,OR_IF,SWIF,NEVERIF';
end;

function TSynLifeScript2HL.GetToken: String;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
function TSynLifeScript2HL.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TSynLifeScript2HL.GetTokenAttribute: TSynHighLighterAttributes;
begin
  case GetTokenID of
...
function TSynLifeScript2HL.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynLifeScript2HL.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

function TSynLifeScript2HL.GetIdentChars: TSynIdentChars;
begin
  Result := ['_', 'a'..'z', 'A'..'Z'];
end;

function TSynLifeScript2HL.GetSampleSource: string;
begin
  Result := '';
end;

function TSynLifeScript2HL.IsFilterStored: Boolean;
begin
  Result := fDefaultFilter <> SYNS_FilterLifeScript;
end;

{$IFNDEF SYN_CPPB_1} class {$ENDIF}
function TSynLifeScript2HL.GetLanguageName: string;
begin
  Result := SYNS_LangLifeScript;
end;

procedure TSynLifeScript2HL.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynLifeScript2HL.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

function TSynLifeScript2HL.GetRange: Pointer;
begin
...
initialization
  MakeIdentTable;
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynLifeScript2HL);
{$ENDIF}
end.
